...
void A2DP_VendorUnloadDecoderLhdcV3(void) { a2dp_vendor_lhdcv3_decoder_cleanup(); }


bool a2dp_vendor_lhdcv3_decoder_init(decoded_data_callback_t decode_callback) {
  LOG_ERROR("[WL50] %s: A2DP Sink", __func__);
...
bool a2dp_vendor_lhdcv3_decoder_decode_packet(BT_HDR* p_buf) {
  ...
  LOG_ERROR("[WL50] %s: data_size (%d) ", __func__, (int) data_size);
  ...
  if (fn_ret == LHDCBT_DEC_FUNC_INPUT_NOT_ENOUGH) {
    LOG_ERROR("[WL50] %s: Input buffer is NOT enough!, but return true", __func__);
    return true;
  }
  ...
  if (packet_bytes != (a2dp_lhdcv3_decoder_cb.dec_input_buf_bytes - LHDCV3_DEC_PKT_HDR_BYTES))
  {
	...
	LOG_ERROR("[WL50] %s: queued data is NOT exactly equal to one packet! packet (%d),  input (%d)", __func__, packet_bytes, a2dp_lhdcv3_decoder_cb.dec_input_buf_bytes);
  ...
  //LOG_ERROR("[WL50] %s: complete decoding lhdc packet", __func__);
